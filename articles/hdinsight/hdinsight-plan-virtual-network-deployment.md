...
> [!IMPORTANT]
> Creating an HDInsight cluster in a VNET will create several networking resources, such as NICs and load balancers. Do **not** modify or delete these networking resources, as they are needed for your cluster to function correctly with the VNET.
...
Another constraint is that the HDInsight load balancers should not be modified by the customer. **Any updates to the load balancer rules will get overwritten during certain maintenance activities such as certificate renewals.** Changes to the load balancer that affect the cluster functionality could result in having to redeploy the cluster.
